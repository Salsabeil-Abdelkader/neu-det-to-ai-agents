...
zero-padding + a sliding 3x3 window), then compares the result against OpenCV's
`cv2.Canny`, which runs the full 5-stage pipeline (Gaussian smoothing → gradient →
non-max suppression → double thresholding → hysteresis).
...
- Canny produces thin, continuous edge curves. The two biggest contributors:
  **non-max suppression** (thins edges to 1px by keeping only local maxima
  along the gradient direction) and **hysteresis** (a weak-gradient pixel only
  survives if connected to a strong one — this is what turns scattered dots
...
- A hand-rolled `apply_kernel` and `cv2.filter2D` compute the same underlying
  ...
  the threshold value used, not the computation method — confirmed directly
  by re-running both with the same percentile-informed threshold.
